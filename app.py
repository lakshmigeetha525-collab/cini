...
import sqlite3
import datetime
import boto3  # Important: install with 'pip install boto3'

app = Flask(__name__)
app.secret_key = 'cine-booker-pangu-ultra-key'

# Locations and Movie Data
locations_data = {
    "Chennai": {
...
# --- AWS SNS Function (No Keys in Code) ---
def send_sns_notification(movie, theatre, seats, date, time):
    try:
        # Boto3 automatic-ah unga local AWS configuration-ai eduthukkum
        sns_client = boto3.client("sns", region_name="ap-south-1")
        
        seat_names = ", ".join([s['id'] for s in seats])
        message = f"Booking Confirmed! Movie: {movie}, Theatre: {theatre}, Seats: {seat_names}, Date: {date}, Time: {time}."
        
        sns_client.publish(
            PhoneNumber="+919345572920", # Inga phone number mattum update pannunga
            Message=message
        )
...
@app.route('/')
def index():
    if 'user_name' in session: return redirect(url_for('dashboard'))
...
def booking(movie_name):
    ...
    selected_date = request.args.get('date') or f"{datetime.date.today().day} {datetime.date.today().strftime('%b')}"
    ...
    with get_db() as conn:
        ...
        for row in rows: booked_seats[row['seat_id']] = row['gender']

    return render_template('booking.html', movie_name=movie_name, theatre=theatre, city=city, 
                           booked_seats=booked_seats, selected_date=selected_date, selected_time=selected_time)
        ...
        with get_db() as conn:
            for seat in data['seats']:
                conn.execute("INSERT INTO bookings (movie, seat_id, gender, date, time, theatre, city) VALUES (?, ?, ?, ?, ?, ?, ?)",
                             (data['movie'], seat['id'], seat['gender'], data['date'], data['time'], data.get('theatre'), data.get('city')))
            conn.commit()
        
        # Calling SNS Function
        send_sns_notification(data['movie'], data.get('theatre'), data['seats'], data['date'], data['time'])
        return jsonify({"status": "success"})
    except Exception as e:
